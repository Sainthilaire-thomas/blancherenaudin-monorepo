...
export async function sendOrderConfirmationHook(orderId: string) {
  try {
   ...
   const supabase = supabaseAdmin
    ...
    // Cast vers le type typé
    const typedOrderItems = orderItems as OrderItem[]

    console.log(`✅ Found ${typedOrderItems.length} items`)
    ...
    if (productImages) {
      productImages.forEach((img: ProductImagePartial) => {
    ...
    const formattedItems = typedOrderItems.map((item: OrderItem) => {
    ...
    const subtotal = typedOrderItems.reduce((sum: number, item: OrderItem) => sum + item.total_price, 0)
